...
- Current broad residue scans still return many `Figure N:` caption lines; that is acceptable for now because captions are retained as textual provenance. The primary remaining non-caption hits should be equation/example lead-ins, not navigation prose that depends on a missing picture.
