...
- The explicit browser-neutral IEA-export note in `data-system.md` is intentionally version-specific: it is preserved where the upstream docs actually depended on the historical `Beyond 2020 Browser` workflow (`v4.3` and `v4.4`), while later versions keep only the generic data-system adaptation header because their source pages moved to the separate `gcamdata` package documentation.
...
- Some historical exact bundles still contain awkward inline-code boundaries inherited from upstream prose. These must be fixed with targeted generator rewrites only; a broad global spacing normalizer is now a known bad approach because it can corrupt literal commands, file names, and validator-pinned adaptation sentences.
- Some historical authoring lines are malformed even before adaptation, for example incomplete inline-code spans or UI-labeled booleans embedded in prose. These should be handled with tolerant generator patterns and regression validators, not by weakening the text-only contract.
