...
- Continue reducing the remaining lower-frequency historical figure residue, which is now mostly limited to text-only `Omitted figure summary:` notes plus preserved equation/example lead-ins rather than raw figure-caption numbering or repeated `shown in Figure N` prose.
- Continue auditing long-tail formatting residue in exact historical pages, but keep the new invariant that fenced code blocks must remain structurally separate from prose lines after generation.
- Continue auditing rare inline-code boundary defects in historical exact pages, but only with narrow source-pattern rewrites; do not reintroduce a repository-wide inline-code spacing pass.
- Keep the broad residue scans clean for old bold/heading figure captions and malformed inline tails such as `..., Figure 3.` or `depicted Figure 11`; those are now treated as generator regressions rather than acceptable historical leftovers.
- Keep broad residue scans clean for other long-tail figure references too, especially mixed table/figure lead-ins and phrases like `presented in the following figure` or `in Figure 1`, which are easy to miss if only caption-style patterns are checked.
...
- 2026-03-16: Added a second Unicode-cleanup pass for presentation-only punctuation, removing smart quotes, Unicode dash variants, thin-space and invisible math separators, and other typography-oriented export residue while keeping semantically meaningful symbols such as Greek variables and `°`.
- 2026-03-16: Normalized raw `Figure N:` caption lines and direct `Figure N` prose references into text-only omitted-figure wording across all bundled versions, including the root-backed `v8.2` pages, and tightened validators so raw figure numbering cannot silently re-enter the published bundle.
- 2026-03-16: Hardened bundled-page link validation to ignore code-fragment pseudo-links without path markers, preventing false missing-target errors from C++ lambda syntax such as `[aName]( auto aClass )` in historically malformed fenced-code regions.
- 2026-03-16: Extended figure normalization to also cover old bold/wiki caption styles and malformed inline figure tails such as `..., Figure 3.` or `depicted Figure 11`, so those older historical residues are now rewritten into text-only omitted-figure wording during generation.
- 2026-03-16: Scoped the explicit `Beyond 2020 Browser` data-system adaptation note to the legacy versions that actually used that workflow (`v4.3` and `v4.4`), while keeping the generic data-system adaptation note for every bundled `data-system.md`.
- 2026-03-16: Added a final generator pass that separates opening markdown code fences from preceding prose, cleaning long-tail historical formatting residue across `style.md`, `rapid_parse.md`, `git.md`, `examples.md`, `test_framework.md`, `en_technologies.md`, and similar exact bundles.
- 2026-03-16: Sanitized the legacy `data-system.md` troubleshooting sample so it no longer preserves the machine-specific Xcode developer `make` path, and expanded bundled-page validation so both attached code fences and that path now fail fast if they reappear.
- 2026-03-16: Normalized historical `dev-guide/style.md` language-scope markers into explicit text labels such as `C++:` and `Java:` across the full-tree version families, improving page readability for agents without hand-editing generated bundles.
- 2026-03-16: Rejected a generic inline-code spacing normalization pass after it broke literal snippets and required adaptation text; future fixes for inline-code boundary residue must stay narrowly targeted and validator-backed.
- 2026-03-16: Added another long-tail exact-page adaptation pass covering `demand_land.md` stockholding schematic text, `v3.2/Regional_Scope_Socioeconomics_and_Trade.md` region-map figure residue, `v3.2/Economic_land_sharing.md` leftover `in Figure 1` wording, and the historical `dev-guide/test_framework.md` checkbox-style `clear_output` explanation.
