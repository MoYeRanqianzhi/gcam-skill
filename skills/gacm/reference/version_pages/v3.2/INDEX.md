# v3.2 Detailed Page Bundle

This directory is the page-level bundled reference set for GCAM `v3.2`.

- Family: `legacy-wiki`
- Coverage mode: `version-summary`
- Source root: `gcam-doc/v3.2`
- Page count: `29`

Progressive-disclosure rule:
- Start from the version route file.
- Open this index only when the user needs page-level detail for this version.
- Then open only the specific page file relevant to the task.

## Bundled Pages

### root
- `Agriculture,_Land-Use,_and_Bioenergy.md`
- `Agriculture_Land-Use_and_Bioenergy.md`
- `Buildings.md`
- `Compiling_GCAM.md`
- `Cycle-breaking_in_GCAM.md`
- `Economic_land_sharing.md`
- `Electricity.md`
- `gcam-build.md`
- `gcam-doc-options.md`
- `GCAM-MPI_Message_Specifications.md`
- `GCAM-parallel.md`
- `GCAM_Build_Configuration.md`
- `GCAM_Model_Parameters.md`
- `GCAM_Revision_History.md`
- `GCAM_Solver.md`
- `Getting_Started_with_GCAM.md`
- `hector.md`
- `index.md`
- `Industry.md`
- `Minimum_system_requirements_for_GCAM.md`
- `Nested_logit_architecture.md`
- `References.md`
- `Regional_Scope_Socioeconomics_and_Trade.md`
- `Resource_Supply_Curves.md`
- `The_Climate_Module.md`
- `The_Energy_System.md`
- `toc.md`
- `Transportation.md`
- `user-guide.md`
