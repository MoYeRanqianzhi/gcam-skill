...
- Source path: `dev-guide/review.md`
...
- Note: Referenced from `v5.4` as `dev-guide/review.md`.
...
After the coding changes are complete, a pull request should be opened either on GitHub (external developers) or on stash (internal developers). Please ensure your developments follow the [GCAM style](style.md) and that the description of your pull request is meaningful. Note that opening the pull request on stash will automatically trigger the [GCAM testing framework](test_framework.md) which will ensure GCAM builds and runs on Windows and Linux and users can launch the validation runs.
