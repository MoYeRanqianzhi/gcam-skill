...
AbatedGasForCostCurves | Default: "CO2".  The name of the market/gas to change the price of and sum emissions for when calculating [Policy Costs](policies.md#policy-costs).  Note when [targeting multiple gasses](policies.md#linked-markets) you may specificy this value as `GHG;CO2;CH4;N2O;C2F6;CF4`, for example, to list all of the gasses to sum and they will be weighted according to the linked ghg policy
...
FixedGDP-Path | Sets if using fixed, or dynamic GDP feedbacks.  See [Economy](economy.md) for more details.
