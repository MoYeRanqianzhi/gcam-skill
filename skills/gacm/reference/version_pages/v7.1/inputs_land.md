# External Inputs to the Land Model
...
| Value of Unmanaged Land | GCAM requires profit rates for all land types in the historical period for calibration. Managed land profit is calculated in the [supply model](inputs_supply.md). For unmanaged land, however, the value is input into the model. | External Data Set | [Moirai](https://github.com/JGCRI/moirai) | Raw data is by country, water basin, and land use. | million USD |
...
<a name="table_footnote"></a>1: Note that this table differs from the one provided on the [Land Modeling Page](land.md#inputs-to-the-module) in that it only lists external inputs to the land module (either data sources or assumptions). Additionally, the units listed are the units of the raw inputs, rather than the units the GCAM requires.
