# The GCAM Land Allocation Module
...
| :--- | :--- | :--- | :--- |
| Historical land use and land cover | By GLU, land type, and year | thousand $$km^2$$ | [Exogenous](inputs_land.md) |
| Vegetation carbon density | By GLU and land type | kg per $$m^2$$ | [Exogenous](inputs_land.md) |
| Soil carbon density | By GLU and land type | kg per $$m^2$$ | [Exogenous](inputs_land.md) |
| Mature age | By GLU and land type | years | [Exogenous](inputs_land.md) |
| Soil time scale | By geopolitical region and land type | years | [Exogenous](inputs_land.md) |
| Value of unmanaged land | By GLU | 1975$ per thous $$km^2$$ | [Exogenous](inputs_land.md) |
...
#### Leaf profit
Profit for managed land leafs is calculated in the [supply module](supply_land.md) and passed to the land allocator. Profit for unmanaged land leafs is [input](inputs_land.md#value-of-unmanaged-land) into the model. Within the land allocator, this profit is adjusted if land-related policies are included.
...
where $$\lambda_i$$ is the [profit scaler](land.md#calibration) for leaf or node $$i$$, $$\pi_i$$ is the [profit](land.md#profit) for node $$i$$, $$\pi_j$$ is the [profit](land.md#profit) for leaf or node $$j$$ contained within node $$i$$, and $$\rho$$ is the [logit exponent](inputs_land.md#share-parameters).
...
where $$s_i$$ is the share of leaf or node $$i$$, $$\lambda_i$$ is the [profit scaler](land.md#calibration) for leaf or node $$i$$, $$\pi_i$$ is the [profit](land.md#profit) for leaf or node $$i$$, and $$\rho$$ is the [logit exponent](inputs_land.md#share-parameters).
...
To calculate land area, GCAM works its way down the [nesting tree](land.md#land-nesting-strategy), starting from the top where total land area in a region is provided as an [input](inputs_land.md#Historical-Land-Cover-and-Use). For each node or leaf below, the area is calculated as
...
To calibrate the land allocator, the [share equation](land.md#shares) is inverted to solve for $$\lambda_i$$.
